...
void RadioData(void *){
	...
	while(1){
		...
		//printf("%d\n",(uint8_t)radio_in_buff[11]);
		OSMboxPost(&Radio_Mail1, (void *)radio_in_buff[24]);
		OSMboxPost(&Radio_Mail2, (void *)radio_in_buff[25]);
		OSMboxPost(&Radio_Mail3, (void *)radio_in_buff[26]);
		OSMboxPost(&Radio_Mail4, (void *)radio_in_buff[27]);
		OSMboxPost(&Radio_Mail5, (void *)radio_in_buff[11]);
		OSMboxPost(&Radio_Mail6, (void *)radio_in_buff[28]);
		OSMboxPost(&Radio_Mail7, (void *)radio_in_buff[29]);
		OSMboxPost(&Radio_Mail8, (void *)radio_in_buff[12]);

		/*if(Radio_mail_rec==NULL){
		//printf("%d\n",(uint8_t)radio_in_buff11[11]);
			CRME=(uint16_t)Radio_Data[28]*256+(uint16_t)Radio_Data[29];
			if((unsigned char)Radio_Data[12]==0 && CRME<60){
				F_range_buff[0]=Radio_Data[24];
				F_range_buff[1]=Radio_Data[25];
				F_range_buff[2]=Radio_Data[26];
				F_range_buff[3]=Radio_Data[27];
				F_range_buff[4]=Radio_Data[32];
				F_range_buff[5]=Radio_Data[33];
				F_range_buff[6]=Radio_Data[12];
				Ant_config=Radio_Data[11];
			}
		}*/
...
void UserMain( void* pd ){
	...
	OSMboxInit(&Radio_Mail1,0);
	OSMboxInit(&Radio_Mail2,0);
	OSMboxInit(&Radio_Mail3,0);
	OSMboxInit(&Radio_Mail4,0);
	OSMboxInit(&Radio_Mail5,0);
	OSMboxInit(&Radio_Mail6,0);
	OSMboxInit(&Radio_Mail7,0);
	...
	*/
	...
	while(1){
		...
		if(FiftyHzflag==1){
			...
			void* Radio_mail_rec1=OSMboxPendNoWait(&Radio_Mail1,&Radio_Data1);
			void* Radio_mail_rec2=OSMboxPendNoWait(&Radio_Mail2,&Radio_Data2);
			void* Radio_mail_rec3=OSMboxPendNoWait(&Radio_Mail3,&Radio_Data3);
			void* Radio_mail_rec4=OSMboxPendNoWait(&Radio_Mail4,&Radio_Data4);
			void* Radio_mail_rec5=OSMboxPendNoWait(&Radio_Mail5,&Radio_Data5);
			void* Radio_mail_rec6=OSMboxPendNoWait(&Radio_Mail6,&Radio_Data6);
			void* Radio_mail_rec7=OSMboxPendNoWait(&Radio_Mail7,&Radio_Data7);
			void* Radio_mail_rec8=OSMboxPendNoWait(&Radio_Mail8,&Radio_Data8);

			if(Radio_mail_rec1==NULL && Radio_mail_rec2==NULL && Radio_mail_rec3==NULL && Radio_mail_rec4==NULL && Radio_mail_rec5==NULL && Radio_mail_rec6==NULL && Radio_mail_rec7==NULL && Radio_mail_rec8==NULL ){
			//printf("%d\n",(uint8_t)radio_in_buff11[11]);
				CRME=(uint16_t)Radio_Data6*256+(uint16_t)Radio_Data7;
				if((unsigned char)Radio_Data8==0 && CRME<60){
					F_range_buff[0]=Radio_Data1;
					F_range_buff[1]=Radio_Data2;
					F_range_buff[2]=Radio_Data3;
					F_range_buff[3]=Radio_Data4;
					Ant_config=Radio_Data5;
				}
			}
