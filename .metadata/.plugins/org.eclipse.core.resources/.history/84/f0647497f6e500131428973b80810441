...
unsigned char* ReadRadio(char* CommandBuff,int radioserial,int buff_size){
	...
	while(loop_true==1){
		...
		//checksum=crc16(in_buff,58);
	//iprintf("checksum=%d,sent checksum=%d\n",checksum,(uint16_t)(in_buff[57]<<8)|(uint16_t)in_buff[58]);
	//	if(checksum==((uint16_t)(in_buff[56]<<8)|(uint16_t)in_buff[57])){
			//iprintf("yes\n");
			if((unsigned char) in_buff[12]==0){
				return in_buff;
			}
		//}
