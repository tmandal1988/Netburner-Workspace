...
void UserMain( void* pd ){
	...
	char RCM_SEND_RANGE_REQUESTB1[]={0xA5,0xA5,0x00,0x0D,0x00,0x03,0x00,0x02,0x00,0x00,0x00,0x65,0x01,0x00,0x00,0x01,0x00,0x5E,0x6C};//Host Antenna B to 101
	char RCM_SEND_RANGE_REQUESTB2[]={0xA5,0xA5,0x00,0x0D,0x00,0x03,0x00,0x02,0x00,0x00,0x00,0x66,0x01,0x00,0x00,0x01,0x00,0x90,0x8C};//Host Antenna B to 102
	...
	uint8_t Radiocount4=0,Radiocount3=0,Ant_config=0;
	//uint32_t Range=0;
	...
	while(1){
		...
		if(Navcomp_in_buff[21]==4){
			...
			if(FiveHzflag==1 && Radiocount4==1){
				radio_in_buff=ReadRadio(RCM_SEND_RANGE_REQUESTA2,fdRadio,sizeof(RCM_SEND_RANGE_REQUESTA2));
				Radiocount4+=1;
				Ant_config=102;
			}//second if bracket

			//Third if statement to command host radio to get ranging data from 101 guest with antenna B
			if(FiveHzflag==2 && Radiocount4==2){
				radio_in_buff=ReadRadio(RCM_SEND_RANGE_REQUESTB1,fdRadio,sizeof(RCM_SEND_RANGE_REQUESTB1));
				Radiocount4+=1;
				Ant_config=101;
			}//third if bracket

			//Fourth if statement to command host radio to get ranging data from 102 guest with antenna B
			if(FiveHzflag==3 && Radiocount4==3){
				radio_in_buff=ReadRadio(RCM_SEND_RANGE_REQUESTB2,fdRadio,sizeof(RCM_SEND_RANGE_REQUESTB2));
				Radiocount4=0;
				Ant_config=103;
			}//fourth if bracket
		}
